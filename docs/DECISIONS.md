...
## 2026-08-01 — Sprint 5: staff TOTP MFA (SECURITY_ARCHITECTURE.md §4.2)

**Decision:** Implement mandatory TOTP MFA for `admin`, `finance`, and
`ops_manager` staff roles, per `SECURITY_ARCHITECTURE.md` §4.2 and
`BACKEND_ARCHITECTURE.md` §16.3. `branch_manager` stays excluded, matching
those documents' explicit single-branch-blast-radius reasoning — no
opt-in path added for it in this PR (out of scope).

**New dependencies (reviewed per `SECURITY_ARCHITECTURE.md` §8 before
adoption):** `otpauth@9.5.1` (TOTP generate/verify + provisioning URI —
MIT, single dependency on `@noble/hashes`, no install/postinstall
scripts) and `qrcode@1.5.4` + `@types/qrcode` (renders the enrollment QR
server-side as a data URL — MIT, standard `pngjs`/`yargs`/`dijkstrajs`
deps, no install/postinstall scripts). No schema migration needed —
`User.totpSecret`/`mfaEnabled` and the `RecoveryCode` model were already
reserved by Sprint 5 PR1.

**Enrollment gating reuses the mustChangePassword pattern.**
`SECURITY_ARCHITECTURE.md` §4.2 says an unenrolled required-role account
"MUST be blocked from completing login until enrollment," but enrollment
itself needs *some* authenticated context to bind the QR code to the
right account — the same chicken-and-egg PR4's `mustChangePassword` gate
already solved. Read the same way here: login succeeds and creates a
real, revocable DB-backed session (`server/auth/staff.ts`'s existing
design, unchanged), but `app/admin/(app)/layout.tsx` redirects everywhere
except `/admin/configurar-mfa` until `mfaEnabled` is true, for the three
required roles. Explicitly confirmed with the project owner before
implementation rather than assumed.

**Two-step login for already-enrolled accounts.** Auth.js's Credentials
provider resolves in one `authorize()` call, and the login UI can't know
in advance whether an account has MFA enrolled. `app/admin/ingresar` is
now a client component (`login-form.tsx`) backed by two server actions
(`app/admin/ingresar/actions.ts`): `checkNeedsSecondFactor` does the same
timing-safe password check `authorize()` uses (`server/auth/staff.ts`'s
`verifyStaffPassword`, extracted so both call sites share one
implementation and one timing-safety story) without creating a session,
purely to decide whether to render the code-entry step; the final
`signInStaff` call sends email+password+totpCode (or recoveryCode)
together into `authorize()`, which independently re-verifies everything
— step 1's result is never trusted for the actual auth decision. Step 1
does reveal "these credentials are valid" as a distinct client-visible
state before the second factor, same as Google/GitHub's own two-step
login UX — an inherent, accepted property of any two-step MFA flow, not
a new vulnerability class.

**`totpSecret` encryption:** AES-256-GCM via Node's built-in `crypto`
(`server/auth/mfa.ts`), keyed by `MFA_ENCRYPTION_KEY` — no new dependency
needed for this half. Key must be a base64-encoded 32-byte value
(`openssl rand -base64 32`); `.env.example` updated to say so explicitly,
since the implementation now depends on that exact format, not just the
conceptual description PR1 left there.

**Recovery codes hashed with SHA-256, not argon2** — confirmed with the
project owner before implementation. They're 40-bit-entropy random
tokens generated by the server, not user-chosen passwords; argon2's
slow-hash design defends against low-entropy guessing, which doesn't
apply here, and would only add real latency to every recovery-code
login. Comparison is still constant-time (`timingSafeEqual`). Codes are
generated once at enrollment confirmation, shown exactly once, and
single-use (`RecoveryCode.usedAt`, enforced via a conditional
`updateMany` that fails closed against a concurrent double-use race).

**Known gap, deliberately not built — flagged rather than silently
skipped:** no replay-window tracking for TOTP codes (RFC 6238 recommends
rejecting a code already consumed within its own validity window, e.g.
against shoulder-surfing). Doing so needs a new column to remember the
last-used time-step per user, which wasn't pre-reserved by PR1's schema
and wasn't covered by the up-front decisions confirmed for this PR — left
as a follow-up requiring its own explicit decision, matching this
document's practice for anything schema-touching, rather than expanding
this PR's scope unilaterally.

**Verification performed:**
- Typecheck (`tsc --noEmit`), lint, and `next build` all clean; new
  `/admin/configurar-mfa` route registered correctly as dynamic,
  matching `/admin/ingresar` and `/admin/cambiar-contrasena`.
- `server/auth/mfa.ts`'s core logic verified functionally with temporary,
  uncommitted scripts (deleted after running, no net `git diff`): TOTP
  round-trip (a freshly generated token validates; wrong/malformed/
  cross-secret tokens don't), AES-256-GCM round-trip (decrypt(encrypt(x))
  === x; two encryptions of the same input produce different ciphertext;
  a tampered ciphertext fails auth-tag verification and throws), and
  recovery-code generation/hashing (10 unique codes, correct format,
  deterministic SHA-256 hashing, case-insensitive normalization matching
  `consumeRecoveryCode`'s own normalization). The `server-only` guard on
  `server/auth/mfa.ts` throws under plain Node/`tsx` execution (confirmed
  directly — same reason `prisma/bootstrap-admin.ts` duplicates
  `ARGON2_OPTIONS` instead of importing it), so these scripts duplicated
  the pure logic rather than importing the guarded module.
- **Live end-to-end verification, performed once a real database was
  available.** The local Postgres this project actually uses (Homebrew,
  port 5432, `morel_dev` — the same one `.env`'s `DATABASE_URL` already
  named) turned out to be reachable the whole time; the earlier "no
  database available" note above was this session's own tooling failing
  to find it (a restricted shell PATH lacking `/opt/homebrew/bin`), not
  an actual absence, and cost real time — worth remembering before
  concluding "no DB" again. Verified against it, driving the real running
  dev server through the Browser tool (not curl/fetch — actual page
  loads, form fills, and clicks) with dedicated test accounts
  (`admin-fresh@example.com`, `finance-fresh@example.com`,
  `branchstaff-fresh@example.com`, all `TestPassword123!`, local dev DB
  only):
  - **mustChangePassword still works, and chains correctly into MFA
    enrollment:** fresh `ADMIN` account (`mustChangePassword: true`) →
    forced to `/admin/cambiar-contrasena` → password changed → forced
    straight to `/admin/configurar-mfa` (same account requires MFA and
    hadn't enrolled) — the two gates compose correctly in sequence.
  - **MFA enrollment → QR → confirmation:** manual key extracted from the
    rendered page, a real TOTP code computed from it via `otpauth`
    (external to the app, simulating an authenticator app), submitted —
    accepted, 10 recovery codes displayed, landed on `/admin` dashboard.
    An intentionally wrong code (`000000`) was rejected first, with the
    same secret still shown afterward (not invalidated by a failed
    attempt).
  - **Login with valid/invalid TOTP:** session cleared, re-authenticated
    with email+password → correctly prompted for the second factor (not
    silently let through) → wrong code (`111111`) rejected inline, zero
    `Session` rows created (confirmed directly in Postgres) → fresh valid
    code accepted, session created, landed on `/admin`.
  - **Recovery code: works once, rejected on reuse, other codes
    unaffected:** one saved code logged in successfully (confirmed
    `RecoveryCode.usedAt` set in the database); the identical code
    rejected on a second attempt with zero `Session` rows created; a
    *different*, still-unused code from the same batch logged in
    successfully immediately after — proving the rejection is genuinely
    per-code single-use, not a blanket failure.
  - **Session revocation:** deleted every `Session` row for an
    authenticated account directly in Postgres, then reloaded an
    already-authenticated `/admin` request — immediately redirected to
    `/admin/ingresar`, confirming `authorize()`'s per-request DB
    re-check (not just the JWT's cached claims) is what's actually
    gating access.
  - **branch_staff (non-MFA-required role) unaffected:** logged in with
    email+password only, landed directly on `/admin` with no second-factor
    prompt and no enrollment redirect — confirms the MFA gate is
    role-scoped, not global.
  - **Customer authentication unaffected:** `/cuenta/ingresar` (Google
    OAuth + magic-link entry point, `server/auth/customer.ts` — untouched
    by this PR) rendered correctly with no console or server errors. Full
    OAuth/magic-link completion was not exercised — this local
    environment has no real `AUTH_GOOGLE_ID`/`AUTH_GOOGLE_SECRET` or
    `RESEND_API_KEY` to complete either flow against — but that's outside
    this PR's actual change surface; the goal here was confirming no
    regression, not re-certifying customer auth itself.
  - Server logs showed exactly two `CredentialsSignin` errors for the
